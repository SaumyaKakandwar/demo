...
function inProgress(ongoing, failed = false, rerun = true) {
  if (ongoing) {
    ...
    document.getElementById('copy-btn').classList.add("invisible");
  } else {
    if (failed) {
...
async function callChatGPT(messages, callback, onDone) {
  ...
  const mockResponses = [
    ...
    - Code looks well-structured. Consider refactoring the use of inline styles to keep the separation of concerns clear.
    - Performance: Ensure that you're not re-rendering the component unnecessarily.`,

    `**File: src/components/App.kt**
    - Code needs some modifications. Consider the use of 'try with resources' instead of 'try' .
    - Performance: Ensure that you're not re-rendering the component unnecessarily.`,
    
    
    `**Overall Cyclomatic Code Complexity**
    The overall cyclomatic complexity is 3. The code has moderate complexity; however, the logic could be simplified in some areas, especially around event handling.`,
    
    `**Improvements Needed**
    - Security: Avoid direct DOM manipulation for security reasons.
    - Performance: Memoize functions to avoid unnecessary recalculations.`,
    
    `**Overall Brief Code Review**
    Code generally follows best practices but has areas for improvement in terms of performance optimizations and security considerations.`
  ];
  ...
  //   }
  // };
    // Simulating the process of going through each message in the array (mocking API progress)
    for (let i = 0; i < messages.length; i++) {
      iterations--;
      try {
        let mockResponse = mockResponses[i % mockResponses.length]; // Cycle through mock responses
  
        // In progress message
        if (iterations > 0) {
          callback("Processing your code changes. Number of prompts left to send: " + iterations + ". Stay tuned...");
        }
  
        // Last prompt (final message)
        if (iterations == 0) {
          callback(mockResponse); // Return the final mocked response
        }
  ...
  // onDone();
  // Simulate the async process to complete after "processing" all messages
  setTimeout(() => {
    onDone(); // Signal that the process is done
  }, 1000);  // Delay to simulate real API timing
}
...
async function reviewPR(diffPath, context, title) {
  ...
  promptArray.push(`The change has the following title: ${title}.
    ...
    Your task is:
    - Review the code changes, specify file name and provide feedback as software enginer.
    - Divide review in sections and mandatorily display review for all these sections where first is file name where changes are made and then there review.
    - Second section cyclomatic complexity of overall code with title "Overall Cyclomatic Code Complexity" and Don't provide cyclomatic complexity of each file and then give code changed cyclomatic complexity in number and State the reason for given complexity with the help of lines of code.
    - Third section Provide line in code where we need to improve security recommendations, performance, code complexity, code quality improvement for code changed.
    - Bold the file names and other titles.
    - Give negative comments as well.
    - If there are any bugs, highlight them.
    - Provide details on missed use of best-practices.
    - Does the code do what it says in the commit messages?
    - Do not highlight minor issues.
    - Use bullet points always.
    - Before providing overall code comment provide that under title "Overall Brief Code Review"
  ...
  callChatGPT(
    promptArray,
    (answer) => {
      // Split the answer by the file names and reviews
      const formattedAnswer = answer.split(/\*\*(.*?)\*\*/g).map((section, index) => {
        if (index % 2 === 1) {
          // File name section (odd index)
          return `
            <div class="file-name-box">
              <strong>${section}</strong>
            </div>`;
        } else {
          // Review section (even index)
          return `
  ...
  document.getElementById("copy-btn").onclick = async() => {
   let text = document.getElementById('result').innerText;
     try {
       ...
       document.getElementById('copy-btn').innerHTML="Failed"
     }
